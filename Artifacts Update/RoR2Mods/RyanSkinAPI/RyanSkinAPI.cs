...
namespace PallesenProductions
{
    ...
    public class RyanSkinAPI : BaseUnityPlugin
    {
        ...
        public void Awake()
        {
            ...
            directory = BepInEx.Paths.PluginPath;

            myConfig = new ConfigFile(BepInEx.Paths.ConfigPath + @"\RyanSkinApi.cfg", true);
            ...
            On.RoR2.SurvivorCatalog.Init += (orig) =>
            {
                ...
                foreach (string pluginFolder in Directory.EnumerateDirectories(directory))
                {
                    foreach (string ModFolder in Directory.EnumerateDirectories(pluginFolder))
                    {
        ...
        public void DumpDefaultSkins(GameObject gameObject)
        {
            if (gameObject.GetComponent<ModelLocator>())
            {
                ...
                {
                    StreamWriter textFile = File.CreateText(directory + @"\DefaultSkinModName\Readme.txt");
                    textFile.Write("Please note that to export this pack, you should zip ONLY the contents of the folder that contains the readme.txt");
                    textFile.Close();

                    string currentDirectory = directory + @"\DefaultSkinModName\plugins\" + currentSkin.name;
